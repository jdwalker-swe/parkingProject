...
public class customerRegisterScreen extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					customerRegisterScreen frame = new customerRegisterScreen();
	...
	public customerRegisterScreen() {
			...
			public void actionPerformed(ActionEvent e) {
				
				{
					...
					//checks if email is already registered in emailDB
					 if(mainWindow.customerEmailDB.size()>=1 && (mainWindow.customerEmailDB.contains(customerEmail.getText()) || mainWindow.customerLoginHash.containsKey(customerUserName.getText())) &&!customerUserName.getText().isBlank() && !customerPassword.getText().isBlank() && !customerEmail.getText().isBlank()) {
					 ...
					 else if(!customerUserName.getText().isBlank() && !customerPassword.getText().isBlank() && !customerEmail.getText().isBlank()) {
							//creating new customer object
							customerLogins login = new customerLogins(customerUserName.getText(),customerPassword.getText(),customerEmail.getText());
							//putting customer email into global DB
							mainWindow.customerEmailDB.add(customerEmail.getText());
							
							//putting customerLogin objects into global DB
							mainWindow.customerDB.add(login);
							
							//putting customer UserNames and PWs into global hashMap
							mainWindow.customerLoginHash.put(customerUserName.getText(), customerPassword.getText());
							
							//print stuff
							//System.out.println("size of DB:" + mainWindow.customerDB.size());
							//System.out.println(customerEmail.getText());
							//System.out.println(mainWindow.customerEmailDB.size());
		...
		lblNewLabel_1.setBounds(157, 72, 122, 109);
