...
public class officerScreen extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					officerScreen frame = new officerScreen();
	...
	public officerScreen() {
		...
		JLabel lblNewLabel = new JLabel("Officer Login");
		...
		CustomerRegisterButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				officerRegisterScreen window5 = new officerRegisterScreen();
				window5.show();
		...
		customerLoginButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					...
					String userID = officerUserName.getText();
					String userPW =officerPassword.getText();
					
					if(mainWindow.officerLoginHash.containsKey(userID)) {
						if(mainWindow.officerLoginHash.get(userID).equals(userPW)) {
							//you logged in
							...
							dispose();
							officerBookingScreen window8 = new officerBookingScreen();
							window8.show();
					...
					else if(officerUserName.getText().isBlank() || officerPassword.getText().isBlank()) {
		...
		lblNewLabel_4.setBounds(377, 79, 131, 146);
